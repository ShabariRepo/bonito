...
- Log retention tier gating: Currently `LOG_RETENTION_DAYS` is a global env var (default 90d) with no tier differentiation. Target: Free=30d, Pro=60d, Enterprise/Scale=90d (full). Retention loop in `log_service.py` needs to read org tier and apply per-org cutoffs instead of global `LOG_RETENTION_DAYS`. Settings UI currently exposes retention to all tiers — should be read-only or hidden for Free/Pro.
...
- **Personal Access Tokens & Project Tokens (2026-05-27):** New `access_tokens` table (migration 044). PATs (`bp-` prefix) carry user permissions and work on ALL endpoints (`/api/*` + `/v1/*`). Project tokens (`bj-` prefix, Pro+) scoped to a single project. Auth dependency (`get_current_user`) handles bp-/bj- prefixes alongside JWT. Gateway `get_auth_context` supports bp- for `/v1/*`. API: `GET/POST /api/tokens`, `DELETE /api/tokens/{id}`, `GET/POST /api/projects/{id}/tokens`. CLI: `bonito auth token create/list/revoke/login`. Frontend: PAT card in Settings. Tier limits: Free=2, Pro=10, Enterprise+=unlimited.

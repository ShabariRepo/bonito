...
- **Studio/Origami billing architecture + admin margin dashboard (2026-06-13):** Confirmed the cost model: orchestration LLM cost pools on the **main Bonito org** (via the shared `ORIGAMI_GATEWAY_KEY`, owned by cat.shabari's org `7b98e7c6`), so **customers never see it** — customers are metered by **turns** (`origami_turn_log`, per their org → tier quota → overage). New: (1) migration 050 adds `cache_read_tokens`/`cache_write_tokens` to `origami_turn_log`; orchestrator accumulates + records them per turn. (2) `metering.cache_aware_cost()` = REAL cost with prompt-cache discounts (reads 0.1x, writes 1.25x) + per-family pricing. `cost_usd` stays full-price (conservative) so the spend cap keeps over-counting — intentional, we recoup via metering. (3) `GET /api/admin/origami/billing` (superadmin) + frontend **Admin → Studio Billing** page: per-org turns vs cap, over/under, overage revenue, `cost_tracked` vs `cost_real` (cache-aware), margin, + fleet totals. Validated: a haiku turn records cr=25407/cw=8469 → real $0.024 vs tracked $0.135 (82% less). **Unit economics confirmed on track:** real cost ~$0.02-0.03/build, overage $0.12/turn → ~75% margin. Earlier cost-panic was self-inflicted: the "$18" was a bad baseline subtraction (real test cost $7.81 tracked / ~$3 real), and tracked cost overstates real by ~2.6x because it doesn't apply the cache discount (proven: tracked $68.08 == full-price $68.14). Cost economics doc: `docs/STUDIO-COST-ECONOMICS.md`.
...
- **Studio/Origami overage pricing — tiered descending rates + heavy-build margin (2026-06-13):** Flat $0.12/turn overage (paid) / $0.10 (Enterprise) is now profitable thanks to caching (typical build-turn ~$0.07 cached Sonnet → ~42% margin), BUT heaviest multi-agent team builds (~$0.13 cached Sonnet, 6–8 iterations) are at/just-over break-even at $0.12. Two levers: (a) **tiered descending overage** — lower tiers pay more per turn (proposed Builder $0.30 / Starter $0.25 / Growth $0.20 / Pro $0.16 / Enterprise $0.12 / Scale $0.10), matching commitment to unit price + nudging upgrades; (b) **gate heavy team builds to Haiku** (~$0.04/turn) so margin holds on every turn. Tie to the Haiku-primary rollout. NOTE: before today's caching work the flat overage was UNDERWATER (~$0.21 cost vs $0.12 charge = −$0.09/turn loss, hidden by the Bedrock $0-cost bug).

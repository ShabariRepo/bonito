...
- **GCS log sink org-partitioned (2026-05-27):** Restructured `gcs_log_sink.py` from flat `logs/{date}/{server}.ndjson` to `{org_id}/{log_type}/{YYYY}/{MM}/{DD}/{HH}.ndjson`. Buffers are now keyed by `(org_id, log_type)` — each org gets isolated folders, each feature (gateway, agent, auth, kb, admin, deployment) gets its own file. Middleware infers `log_type` from URL path. Enables per-org GCS lifecycle rules for tier-based retention and clean Helios ingestion per feature.

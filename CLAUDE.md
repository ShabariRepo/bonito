...
- **KNOWN OPEN: Bedrock tool-call text-mode bug (2026-06-13):** When the orchestrator routes Claude through Bonito's gateway to AWS Bedrock (streaming), the model returns TEXT ("I've set up the project…") instead of emitting tool calls — so builds claim success but create nothing (0/3 deterministic locally). Isolated: direct LiteLLM streaming with tools tool-calls fine; the org's LiteLLM Router tool-calls fine; only the full gateway HTTP streaming path text-modes. Cause not yet root-caused (something the gateway/streaming layer or orchestrator request-assembly adds for the Bedrock route). Prod Studio runs Anthropic-direct so it's unaffected (20/20), but Bedrock-connected customers + local dev are exposed, and text-moding triggers committed-without-invoke retries that amplify spend. NOT fixed yet.

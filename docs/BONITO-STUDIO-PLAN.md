...
| **2 — Frontend shell** | Jun 22–26 | New `/` route, full-bleed chat layout, collapsed sidebar with 7-item grouped nav. Move existing dashboard view to `/dashboard`. Persistent thread per `(user, org)`, snapshot-aware first message rendered server-side. First-login migration tooltip. **Phase 2A shipped 2026-06-12 (c557aa6):** /studio route live, snapshot-aware opener with 4 patterns, login + SSO redirect to /studio, sidebar default-collapsed, Studio nav item added. **Phase 2B pending:** full domain regroup of sidebar into 6 sections (Agents / Knowledge / Gateway / Integrations / Team / Settings) — deferred to limit blast radius. |
